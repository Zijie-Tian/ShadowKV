...
class ShadowKVCache:
    """ShadowKV accuracy/debug cache implementation for batch_size=1."""
    def __init__(self, 
        config :object,
        batch_size :int = 1,
        max_length :int = 32*1024, 
    ...
    def prefill_kv_cache(self,
        ...
        # Short LongBench prompts can have fewer retrievable landmark chunks than
        # the static sparse budget. Clamp only the active budget for this prompt
        # so top-k never asks for more chunks than are available.
        ...
        if self.chunks == 0:
            ...
            empty_landmarks = key_states_roped.new_empty(
                self.batch_size, self.num_key_value_heads, 0, self.head_dim
            )
            empty_idx = torch.empty(
                self.batch_size, self.num_key_value_heads, 0,
        ...
        outlier_chunk_k_cache = key_states_roped_ctx.gather(dim=2, index=outlier_chunk_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, self.chunk_size, self.head_dim)).view(self.batch_size, self.num_key_value_heads, self.active_outlier_chunk*self.chunk_size, self.head_dim)
        
        outlier_chunk_v_cache = new_v_cache[:,:,:self.chunks*self.chunk_size].view(self.batch_size, self.num_key_value_heads, self.chunks, self.chunk_size, self.head_dim).gather(dim=2, index=outlier_chunk_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, self.chunk_size, self.head_dim)).view(self.batch_size, self.num_key_value_heads, self.active_outlier_chunk*self.chunk_size, self.head_dim)
